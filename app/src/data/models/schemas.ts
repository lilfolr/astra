...
export const StarshipSchema = v.object({
  ...
  primaryCaptainId: v.pipe(
    v.string('Primary Captain ID must be a string'),
    v.nonEmpty('Primary Captain ID is required'),
  ),
  /** Display name of the starship */
  name: v.pipe(
    v.string('Starship name must be a string'),
    v.nonEmpty('Starship name is required'),
  ),
...
export const ModuleSchema = v.object({
  ...
  incompleteMissions: v.array(
    ...
    'Incomplete missions must be an array of strings',
  ),
});
...
export const CrewSchema = v.object({
  ...
  credits: v.pipe(
    v.number('Credits must be a number'),
    v.minValue(0, 'Credits cannot be negative'),
  ),
  /** Experience points earned */
  xp: v.pipe(
    v.number('XP must be a number'),
    v.minValue(0, 'XP cannot be negative'),
  ),
  /** Current level of the crew member */
  level: v.pipe(
    v.number('Level must be a number'),
    v.minValue(1, 'Level must be at least 1'),
  ),
